--
--  \brief  Driver for the IRQ controller of the platform
--  \author Martin stein
--  \date   2020-02-02
--
...
private
   ...
   type Stored_APIC_ID_Type  is range 0 .. 255;
   ...
   --
   type LAPIC_ID_Type is record
      APIC_ID : Bitfield_8_Type;
   end record;

   type LAPIC_ID_Reg_Type is new LAPIC_ID_Type
   with Size => 32, Atomic;
   ...
   with Address => System'To_Address (LAPIC_Virt_Addr + 16#20#);

   --
   --  Local APIC, end of interrupt register (strict write)
   --
   LAPIC_EOI : Register_32_Type
   with Address => System'To_Address (LAPIC_Virt_Addr + 16#b0#);

   --
   --  Local APIC, spurious interrupt vector register
   --
   type LAPIC_SVR_Type is record
      APIC_Enable : Bitfield_1_Type;
   end record;

   type LAPIC_SVR_Reg_Type is new LAPIC_SVR_Type
   with Size => 32, Atomic;
   ...
   with Address => System'To_Address (LAPIC_Virt_Addr + 16#f0#);

   --
   --  Local APIC, in-service registers
   --
   LAPIC_ISR_0 : Register_32_Type
   with Address => System'To_Address (LAPIC_Virt_Addr + 16#100#);

   LAPIC_ISR_1 : Register_32_Type
   with Address => System'To_Address (LAPIC_Virt_Addr + 16#110#);

   LAPIC_ISR_2 : Register_32_Type
   with Address => System'To_Address (LAPIC_Virt_Addr + 16#120#);

   LAPIC_ISR_3 : Register_32_Type
   with Address => System'To_Address (LAPIC_Virt_Addr + 16#130#);

   LAPIC_ISR_4 : Register_32_Type
   with Address => System'To_Address (LAPIC_Virt_Addr + 16#140#);

   LAPIC_ISR_5 : Register_32_Type
   with Address => System'To_Address (LAPIC_Virt_Addr + 16#150#);

   LAPIC_ISR_6 : Register_32_Type
   with Address => System'To_Address (LAPIC_Virt_Addr + 16#160#);

   LAPIC_ISR_7 : Register_32_Type
   with Address => System'To_Address (LAPIC_Virt_Addr + 16#170#);

   --
   --  Local APIC, interrupt command register (strict write)
   --
   type LAPIC_ICR_Low_Type is record
      Vector          : Bitfield_8_Type;
      Delivery_Status : Bitfield_1_Type;
      Level_Assert    : Bitfield_1_Type;
   end record;

   type LAPIC_ICR_Low_Reg_Type is new LAPIC_ICR_Low_Type
   with Size => 32, Atomic;
   ...
   with Address => System'To_Address (LAPIC_Virt_Addr + 16#300#);

   type LAPIC_ICR_High_Type is record
      Destination : Bitfield_8_Type;
   end record;

   type LAPIC_ICR_High_Reg_Type is new LAPIC_ICR_High_Type
   with Size => 32, Atomic;
   ...
   with Address => System'To_Address (LAPIC_Virt_Addr + 16#310#);
   ...
   IOAPIC_IORegSel : Register_32_Type
   with Address => System'To_Address (IOAPIC_Virt_Addr + 16#0#);

   --
   --  I/O APIC, I/O APIC version register
   --
   type IOAPIC_IOAPICVer_Type is record
      Max_Redirection_Entry : Bitfield_8_Type;
      APIC_Version          : Bitfield_8_Type;
   end record;

   type IOAPIC_IOAPICVer_Reg_Type is new IOAPIC_IOAPICVer_Type
   with Size => 32, Atomic;
   ...
   with Address => System'To_Address (IOAPIC_IOWin_Virt_Addr);

   --
   --  I/O APIC, interrupt redirection table entry
   --
   type IOAPIC_IORedTbl_Low_Type is record
      Vector           : Bitfield_8_Type;
   ...
   type IOAPIC_IORedTbl_Low_Reg_Type is new IOAPIC_IORedTbl_Low_Type
   with Size => 32, Atomic;
   ...
   type IOAPIC_IORedTbl_High_Type is record
      Destination_Field : Bitfield_8_Type;
   end record;

   type IOAPIC_IORedTbl_High_Reg_Type is new IOAPIC_IORedTbl_High_Type
   with Size => 32, Atomic;
