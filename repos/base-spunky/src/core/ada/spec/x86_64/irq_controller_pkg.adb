...
package body IRQ_Controller_Pkg is
   ...
   begin
      ...
      IOAPIC_IORegSel := IOAPIC_IORegSel_IOAPICVer;
      Ctrl.Number_Of_IORedTbl_Regs :=
         Number_Of_IORedTbl_Regs_Type (
            IOAPIC_IOAPICVer_Type (IOAPIC_IOAPICVer_Reg)
            .Max_Redirection_Entry + 1);
      ...
      for Idx in IRQ_Index_Type'Range loop
         ...
         if Idx < IRQ_Index_Type (Ctrl.Number_Of_IORedTbl_Regs) then
            ...
            Declare_IORedTbl :
            declare
               IORedTbl_Low  : IOAPIC_IORedTbl_Low_Type;
               IORedTbl_High : IOAPIC_IORedTbl_High_Type;
            begin
               ...
               IOAPIC_IORegSel :=
                  Register_32_Type (IOAPIC_IORegSel_IORedTbl + 2 * Idx + 1);

               IOAPIC_IORedTbl_High_Reg :=
                  IOAPIC_IORedTbl_High_Reg_Type (IORedTbl_High);

               IOAPIC_IORegSel :=
                  Register_32_Type (IOAPIC_IORegSel_IORedTbl + 2 * Idx);

               IOAPIC_IORedTbl_Low_Reg :=
                  IOAPIC_IORedTbl_Low_Reg_Type (IORedTbl_Low);
      ...
      Declare_LAPIC_SVR :
      declare
         LAPIC_SVR : LAPIC_SVR_Type := LAPIC_SVR_Type (LAPIC_SVR_Reg);
      begin

         LAPIC_SVR.APIC_Enable := 1;
         LAPIC_SVR_Reg := LAPIC_SVR_Reg_Type (LAPIC_SVR);
   ...
   begin
      Bit := Builtin_FFS (Unsigned_32 (LAPIC_ISR_0));
      if Bit /= 0 then
         return Vec_Base + Bit;
      end if;
      Vec_Base := Vec_Base + 32;

      Bit := Builtin_FFS (Unsigned_32 (LAPIC_ISR_1));
      if Bit /= 0 then
         return Vec_Base + Bit;
      end if;
      Vec_Base := Vec_Base + 32;

      Bit := Builtin_FFS (Unsigned_32 (LAPIC_ISR_2));
      if Bit /= 0 then
         return Vec_Base + Bit;
      end if;
      Vec_Base := Vec_Base + 32;

      Bit := Builtin_FFS (Unsigned_32 (LAPIC_ISR_3));
      if Bit /= 0 then
         return Vec_Base + Bit;
      end if;
      Vec_Base := Vec_Base + 32;

      Bit := Builtin_FFS (Unsigned_32 (LAPIC_ISR_4));
      if Bit /= 0 then
         return Vec_Base + Bit;
      end if;
      Vec_Base := Vec_Base + 32;

      Bit := Builtin_FFS (Unsigned_32 (LAPIC_ISR_5));
      if Bit /= 0 then
         return Vec_Base + Bit;
      end if;
      Vec_Base := Vec_Base + 32;

      Bit := Builtin_FFS (Unsigned_32 (LAPIC_ISR_6));
      if Bit /= 0 then
         return Vec_Base + Bit;
      end if;
      Vec_Base := Vec_Base + 32;

      Bit := Builtin_FFS (Unsigned_32 (LAPIC_ISR_7));
      if Bit /= 0 then
         return Vec_Base + Bit;
      end if;
      Vec_Base := Vec_Base + 32;
   ...
   begin
      ...
      --
      IOAPIC_IORegSel :=
         Register_32_Type (IOAPIC_IORegSel_IORedTbl + (2 * IRQ_Idx));

      Declare_IORedTbl_Low :
      declare
         IORedTbl_Low : IOAPIC_IORedTbl_Low_Type :=
            IOAPIC_IORedTbl_Low_Type (IOAPIC_IORedTbl_Low_Reg);
      begin
         if Mask then
            IORedTbl_Low.Mask := 1;
         else
   ...
   is
      ...
      ICR_High : LAPIC_ICR_High_Type;
      ICR_Low  : LAPIC_ICR_Low_Type;
   begin

      Wait_For_Delivery_Status :
      loop

         exit Wait_For_Delivery_Status when
            LAPIC_ICR_Low_Type (LAPIC_ICR_Low_Reg).Delivery_Status = 0;
      ...
      LAPIC_ICR_High_Reg := LAPIC_ICR_High_Reg_Type (ICR_High);
      LAPIC_ICR_Low_Reg  := LAPIC_ICR_Low_Reg_Type (ICR_Low);
   ...
   begin
      ...
      Stored_APIC_IDs (CPU_Idx) :=
         Stored_APIC_ID_Type (
            LAPIC_ID_Type (LAPIC_ID_Reg).APIC_ID);
