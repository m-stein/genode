...
class Genode::Xml_node
{
	...
	public:
		...
		T attribute_value(char const *type, T const default_value) const
		{
			...
			with_optional_attribute(type, [&] (Xml_attribute const &attr) {
				attr.value(result); });
			return result;
		}

		/**
		 * Apply functor 'found_fn' to attribute of specified type
		 *
		...
		/**
		 * Apply functor 'fn' to attribute of specified type
		 *
		 * The functor is called with the attribute as argument. If no matching
