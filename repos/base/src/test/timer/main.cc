...
struct Lazy_test
{
	...
	{
		fast_timer.xrigger_once(TIMEOUT_US);
		faster_timer.xrigger_once(TIMEOUT_US/FACTOR);
		faster ++;
	}
	...
	Lazy_test(Env &env, Signal_context_capability done) : env(env), done(done)
	{
		...
		slow_timer.xrigger_once(RUN_TIME_US);
		set_fast_timers();
	}
};
...
struct Stress_test
{
	...
	struct Slave
	{
		...
		void start() { timer.xrigger_once(us); }
	...
	void handle()
	{
		if (count < DURATION_SEC) {
			...
			timer.xrigger_once((uint64_t)1000 * 1000);
		} else {
