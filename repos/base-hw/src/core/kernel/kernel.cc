...
extern "C" void kernel()
{
	...
	auto const processor_id = Processor::executing_id();
	auto const processor = processor_pool()->processor(processor_id);
	auto const scheduler = processor->scheduler();
	 ...
	 */
	auto const old_client = scheduler->occupant();
	 ...
	 * processor occupant.
	 */
	auto const new_client = scheduler->update_occupant();
	auto const new_state = new_client->lazy_state();
