...
void Thread::ipc_send_request_succeeded()
{
	...
	user_arg_0(0);
	_state = ACTIVE;
	if (!Cpu_job::own_scheduling_context_active()) {
		_activate_used_scheduling_contexts();
	}
}
...
void Thread::_deactivate_used_scheduling_contexts()
{
	Cpu_job::_deactivate_own_scheduling_context();
	_ipc_node.for_each_helper([&] (Thread &thread) {
		thread._deactivate_used_scheduling_contexts(); });
}


void Thread::_activate_used_scheduling_contexts()
{
	Cpu_job::_activate_own_scheduling_context();
	_ipc_node.for_each_helper([&] (Thread &thread) {
		thread._activate_used_scheduling_contexts(); });
}
...
{
	if (_state == ACTIVE && !_paused) {
		_deactivate_used_scheduling_contexts();
	}
	_state = s;
}
...
void Thread::_call_send_request_msg()
{
	...
	_state = AWAITS_IPC;
	if (!help || !dst->own_scheduling_context_active()) {
		_deactivate_used_scheduling_contexts();
	}
}
