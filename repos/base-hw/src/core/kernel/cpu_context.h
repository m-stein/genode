...
class Kernel::Cpu_job : private Scheduling_context
{
	...
	protected:
		 ...
		 */
		void _activate_own_scheduling_context();

		/**
		 * Deactivate our own scheduling context
		 */
		void _deactivate_own_scheduling_context();

		/**
		 * Yield the currently scheduled scheduling context of this context
		 */
		void _yield();

		/**
		 * Return wether we are allowed to help a job with our scheduling context
		 */
	...
	public:
		 ...
		 */
		bool own_scheduling_context_active() { return Scheduling_context::ready(); }
