...
class Scheduler_test::Main
{
	private:
		...
		enum { MAX_NR_OF_CONTEXTS = 10 };
		enum { IDLE_CONTEXT_ID = 0 };
		enum { INVALID_CONTEXT_ID = MAX_NR_OF_CONTEXTS };

		Env                               &_env;
		Scheduling_context                 _idle_context                 { 0, 0 };
		Constructible<Scheduling_context>  _contexts[MAX_NR_OF_CONTEXTS] { };
		time_t                             _current_time                 { 0 };
		Scheduler                          _scheduler;

		unsigned _id_of_context(Scheduling_context const &context) const
		{
			if (&context == &_idle_context) {

				return IDLE_CONTEXT_ID;
			}
			unsigned result { INVALID_CONTEXT_ID };
			for (unsigned id { 0 }; id < sizeof(_contexts)/sizeof(_contexts[0]);
			     id++) {
			...
			if (result == INVALID_CONTEXT_ID) {

				error("failed to determine ID of context");
				_env.parent().exit(-1);
			}
			return 0;
		}

		Scheduling_context &_context(unsigned const id)
		{
			if (id == IDLE_CONTEXT_ID) {

				return _idle_context;
			}
			if (id >= sizeof(_contexts)/sizeof(_contexts[0])) {

				error("failed to determine context by ID");
				_env.parent().exit(-1);
			}
			return *_contexts[id];
		}
		                        ...
		                        unsigned const line_nr)
		{
			if (id == IDLE_CONTEXT_ID ||
				...
				error("failed to construct context in line ", line_nr);
				_env.parent().exit(-1);
			}
			if (_contexts[id].constructed()) {
		                      ...
		                      unsigned const line_nr)
		{
			if (id == IDLE_CONTEXT_ID ||
				...
				error("failed to destroy context in line ", line_nr);
				_env.parent().exit(-1);
			}
			if (!_contexts[id].constructed()) {
		                            ...
		                            unsigned const line_nr)
		{
			...
			Scheduling_context &head { _scheduler.head() };
			unsigned const head_quota { _scheduler.head_quota() };
			if (&head != &_context(expected_head_id)) {

				error("wrong head ", _id_of_context(head), " in line ",
				      line_nr);
		...
		void _set_context_ready_and_check(unsigned const context_id,
		                                  bool     const expect_head_outdated,
		                                  unsigned const line_nr)
		{
			_scheduler.ready(_context(context_id));
