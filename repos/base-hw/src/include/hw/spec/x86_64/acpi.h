...
struct Hw::Apic_madt
{
	...
	struct Ioapic : Genode::Mmio<0xc>
	{
		...
		Ioapic(Apic_madt const * a) : Mmio({(char *)a, REGISTER_SET_SIZE}) { }
	};
	...
	struct Lapic : Genode::Mmio<0x8>
	{
		...
		Lapic(Apic_madt const * a) : Mmio({(char *)a, REGISTER_SET_SIZE}) { }
...
struct Hw::Acpi_fadt : Genode::Mmio<276>
{
	...
	Acpi_fadt(Acpi_generic const * a) : Mmio({(char *)a, REGISTER_SET_SIZE}) { }
...
struct Hw::Acpi_facs : Genode::Mmio<64>
{
	...
	Acpi_facs(addr_t const mmio) : Mmio({(char *)mmio, REGISTER_SET_SIZE}) { }
};
