...
struct Vfs_tresor::Initialized_tresor_adapter_interface
{
	...
	virtual void snapshots_fs_update_snapshot_registry() = 0;
...
class Vfs_tresor::Tresor_adapter : Client_data_interface, Crypto_key_files_interface, Initialized_tresor_adapter_interface
{
	private:
		...
		void snapshots_fs_update_snapshot_registry() override;
				...
				Write_result _try_complete_write(size_t src_num_bytes, size_t &out_count)
				{
					...
					if (_write->complete()) {
						if (_write->success()) {
							...
							result = WRITE_ERR_IO;
							if (VERBOSE)
								log("write (seek ", seek(), " size ", src_num_bytes, ") failed");
						}
						_write.destruct();
						_state = INIT;
					} else {
				...
				Read_result _try_complete_read(size_t dst_num_bytes, size_t &out_count)
				{
					...
					if (_read->complete()) {
						if (_read->success()) {
							...
							result = READ_ERR_IO;
							if (VERBOSE)
								log("read (seek ", seek(), " size ", dst_num_bytes, ") failed");
						}
						_read.destruct();
						_state = INIT;
					} else {
					...
					_adapter.with_initialized_interface([&] (Initialized_tresor_adapter_interface &adapter) {
						...
						case INIT:
						{
							if (adapter.exceeds_data_file_range(seek(), dst.num_bytes)) {
								result = READ_ERR_IO;
								if (VERBOSE)
									log("read (seek ", seek(), " size ", dst.num_bytes, ") exceeds file");
								break;
							}
					...
					_adapter.with_initialized_interface([&] (Initialized_tresor_adapter_interface &adapter) {
						...
						case INIT:
						{
							if (adapter.exceeds_data_file_range(seek(), src.num_bytes)) {
								result = WRITE_ERR_IO;
								if (VERBOSE)
									log("write (seek ", seek(), " size ", src.num_bytes, ") exceeds file");
								break;
							}
...
void Vfs_tresor::Tresor_adapter::snapshots_fs_update_snapshot_registry()
{
