	...
	if (child_state == Channel::READ_BLOCK) {
		...
		if (!child.valid()) {
			...
			if (chan._nr_of_leaves == 0) {
				...
				if (VERBOSE_CHECK)
					log("    lvl ", lvl, " child ", child_idx, " unused");

			} else {

				if (VERBOSE_CHECK)
					log("    lvl ", lvl, " child ", child_idx, " unexpectedly in use");
		...
		if (child.gen == INITIAL_GENERATION ||
			...
			if (VERBOSE_CHECK)
				log("    lvl ", lvl, " child ", child_idx, " has good hash");

		} else {

			if (VERBOSE_CHECK)
				log("    lvl ", lvl, " child ", child_idx, " has bad hash");
...
void Ft_check::_execute_leaf_child(Channel                 &chan,
                                   Type_2_node_block_index  child_idx,
                                   bool                    &progress)
{
	...
	if (child_state == Channel::READ_BLOCK) {
		...
		if (chan._nr_of_leaves == 0) {
			...
			if (child.valid()) {
				...
				if (VERBOSE_CHECK)
					log("    lvl 1 child ", child_idx, " unexpectedly in use");
				...
				if (VERBOSE_CHECK)
					log("    lvl 1 child ", child_idx, " unused");
			}
			...
			if (VERBOSE_CHECK)
				log("    lvl 1 child ", child_idx, " done");
                              ...
                              bool    &progress)
{
	...
	for (Type_2_node_block_index child_idx { 0 };
	...
	for (Tree_level_index lvl { FIRST_FT_T1_LVL_IDX }; lvl <= req._max_lvl; lvl++) {

		for (Type_1_node_block_index child_idx { 0 };
		     child_idx <= req._max_child_idx;
		     child_idx++) {
