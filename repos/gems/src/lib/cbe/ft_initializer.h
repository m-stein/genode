...
#ifndef _FT_INITIALIZER_H_
#define _FT_INITIALIZER_H_
...
class Cbe::Ft_initializer_request : public Module_request
{
	public:
...
class Cbe::Ft_initializer_channel
{
	private:
		...
		struct Type_2_level
		{
			...
			Child_state       children_state[NR_OF_TYPE_2_NODES_PER_BLK] { DONE };
		};
		...
		static void reset_node(Cbe::Type_2_node &node)
		{
		...
		static void reset_level(Type_2_level &level,
		                        Child_state   state)
		{
			for (unsigned int i = 0; i < NR_OF_TYPE_2_NODES_PER_BLK; i++) {
				reset_node(level.children.nodes[i]);
				level.children_state[i] = state;
			}
		}
...
class Cbe::Ft_initializer : public Module
{
	private:
