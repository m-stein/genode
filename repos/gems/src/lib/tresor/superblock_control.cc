                                           ...
                                           bool             &progress)
{
	...
	case Channel::State::SUBMITTED:
		...
		if (VERBOSE_READ_VBA)
			log("read vba ", channel._req_ptr->_vba,
                                                 ...
                                                 size_t   buf_size)
{
	for (Channel &channel : _channels) {
		if (channel._req_ptr && channel._state == Channel::COMPLETED) {
			if (sizeof(channel._req_ptr) > buf_size) {
				class Exception_1 { };
				throw Exception_1 { };
			}
			memcpy(buf_ptr, channel._req_ptr, sizeof(*channel._req_ptr));
			return true;
		}
	}
	return false;
