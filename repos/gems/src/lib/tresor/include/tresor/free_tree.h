...

{
		...
		for (unsigned idx { 0 }; idx < TREE_MAX_NR_OF_LEVELS; idx++) {
			...
			if (!(*(Tree_walk_pbas *)_new_blocks_ptr).pbas[idx])
				continue;

			//Genode::print(out, first ? "" : " ", idx, ":", pbas[idx]);
			log(first ? "   pba " : "   pba ",(*(Tree_walk_pbas *)_new_blocks_ptr).pbas[idx]);
			if (first)
				first = false;
		}
}
{
		...
		for (unsigned idx { 0 }; idx < TREE_MAX_NR_OF_LEVELS; idx++) {
			...
			if (!(*(Type_1_node_walk *)_old_blocks_ptr).nodes[idx].valid())
				continue;

			//Genode::print(out, first ? "" : " ", idx, "[", nodes[idx], "]");
			log(first ? "   Node " : "   Node ", (*(Type_1_node_walk *)_old_blocks_ptr).nodes[idx]);
			if (first)
				first = false;
		}
}
