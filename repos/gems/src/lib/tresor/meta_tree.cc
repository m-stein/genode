	...
	for (uint32_t id { 0 }; id < NR_OF_CHANNELS; id++) {
		...
		if (local_req.state == Local_cache_request::PENDING) {
			...
			construct_in_buf<Block_io_request>(
				...
				(void *)&channel._cache_request.encoded_blk, nullptr);
	...
	case Local_cache_request::READ:
		...
		if (local_req.level > T2_NODE_LVL) {
			...
			if (!check_sha256_4k_hash(channel._cache_request.encoded_blk, t1_info.node.hash)) {
				...
				t1_info.entries.decode_from_blk(channel._cache_request.encoded_blk);
				t1_info.index = 0;
			...
			if (!check_sha256_4k_hash(channel._cache_request.encoded_blk, t2_info.node.hash)) {
				...
				t2_info.entries.decode_from_blk(channel._cache_request.encoded_blk);
				t2_info.index = 0;
                                     ...
                                     bool    &handled)
{
	...
	{
		Block encoded_blk { };
		t2_info.entries.encode_to_blk(encoded_blk);

		_update_parent(
			t1_info.entries.nodes[t1_info.index], encoded_blk,
			req._current_gen, t2_info.node.pba);

		channel._cache_request = Local_cache_request {
			Local_cache_request::PENDING, Local_cache_request::WRITE, false,
			t2_info.node.pba, 1, &encoded_blk };
                                      ...
                                      bool    &handled)
{
	...
	for (uint64_t lvl { MT_LOWEST_T1_LVL }; lvl <= TREE_MAX_LEVEL; lvl++) {
		...
		{
			Block encoded_blk;
			t1_info.entries.encode_to_blk(encoded_blk);
			...
			if (lvl == req._mt_max_lvl) {
				...
				_update_parent(
					root_node, encoded_blk, req._current_gen,
					t1_info.node.pba);
				...
				*(uint64_t *)req._mt_root_gen_ptr = root_node.gen;
				...
				_update_parent(
					parent.entries.nodes[parent.index], encoded_blk,
			...
			channel._cache_request = Local_cache_request {
				...
				false, t1_info.node.pba, lvl, &encoded_blk };
