...
bool Sb_check::Check::execute(Vbd_check &vbd_check, Ft_check &ft_check, Block_io &block_io)
{
	...
	case INIT:
		...
		_read_block.construct(_helper, READ_BLK, READ_BLK_SUCCEEDED, progress, _sb_idx, _blk);
		break;
	...
	case READ_BLK_SUCCEEDED:
		...
		if (_scan_for_highest_gen_sb_done) {
			...
			if (snap.valid) {
				...
				_check_vbd.construct(_helper, CHECK_VBD, CHECK_VBD_SUCCEEDED, progress, *_tree_root);
				if (VERBOSE_CHECK)
			...
			if (_sb_idx < MAX_SUPERBLOCK_INDEX) {
				...
				_read_block.construct(_helper, READ_BLK, READ_BLK_SUCCEEDED, progress, _highest_gen_sb_idx, _blk);
				if (VERBOSE_CHECK)
	...
	case CHECK_VBD_SUCCEEDED:
		...
		if (_snap_idx < MAX_SNAP_IDX) {
			...
			_check_ft.construct(_helper, CHECK_FT, CHECK_FT_SUCCEEDED, progress, *_tree_root);
			if (VERBOSE_CHECK)
				log("  check free tree");
		}
		break;
	...
	case CHECK_FT_SUCCEEDED:
		...
		_check_ft.construct(_helper, CHECK_MT, CHECK_MT_SUCCEEDED, progress, *_tree_root);
		if (VERBOSE_CHECK)
			log("  check meta tree");
		break;
