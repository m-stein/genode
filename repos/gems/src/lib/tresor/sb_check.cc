...
bool Sb_check::Check::execute(Vbd_check &vbd_chk, Ft_check &ft_chk, Block_io &blk_io)
{
	...
	case INIT:
		...
		_generate_req(_read_blk, READ_BLK_SUCCEEDED, progress, _sb_idx, _blk);
		break;
	...
	case READ_BLK_SUCCEEDED:
		...
		if (_scan_for_highest_gen_sb_done) {
			...
			if (snap.valid) {
				...
				_generate_req(_check_vbd, CHK_VBD_SUCCEEDED, progress, *_tree_root);
				if (VERBOSE_CHECK)
			...
			if (_sb_idx < MAX_SUPERBLOCK_INDEX) {
				...
				_generate_req(_read_blk, READ_BLK_SUCCEEDED, progress, _highest_gen_sb_idx, _blk);
				if (VERBOSE_CHECK)
	...
	case CHK_VBD_SUCCEEDED:
		...
		if (_snap_idx < MAX_SNAP_IDX) {
			...
			_generate_req(_check_ft, CHK_FT_SUCCEEDED, progress, *_tree_root);
			if (VERBOSE_CHECK)
				log("  check free tree");
		}
		break;
	...
	case CHK_FT_SUCCEEDED:
		...
		_generate_req(_check_ft, CHK_MT_SUCCEEDED, progress, *_tree_root);
		if (VERBOSE_CHECK)
			log("  check meta tree");
		break;

	case CHK_MT_SUCCEEDED: _mark_req_successful(progress); break;
	default: break;
	}
	return progress;
}
...
void Sb_check::Check::_mark_req_failed(bool &progress, char const *str)
{
...
void Sb_check::Check::_mark_req_successful(bool &progress)
{
