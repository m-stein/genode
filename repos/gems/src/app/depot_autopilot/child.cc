...
bool Log_event::handle_log_update(Expanding_string const &log_str)
{
	while (true) {
		...
		if (_log_prefix_valid) {
			 ...
			 * for this event, the chunk is treated as a sequence of bytes
			 * that do not even partially match the pattern. In this case,
			 * the entire log chunk is skipped and an already existing partial
			 * match of the pattern chunk is discarded.
			 */
...
:
	Event             { xml, Type::LOG },
	_alloc            { alloc },
	_log_prefix       { xml.attribute_value("log_prefix", Log_prefix { }) },
