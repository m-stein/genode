	...
	private Dynamic_rom_session::Xml_producer
{
	private:
		...
		bool                                   _startup_retry                      { false };
		size_t                                 _cbe_image_size                     { 0 };
		size_t                                 _client_fs_size                     { 0 };
		bool                                   _client_fs_in_use                   { false };
		...
		static bool _child_provides_session(Xml_node    const &sandbox_state,
		                                    Child_state const &child_state,
...
bool File_vault::Main::_child_provides_session(Xml_node    const &sandbox_state,
                                               Child_state const &child_state,
                                               String<64>  const &service_name)
{
	bool result { false };
...
void File_vault::Main::handle_sandbox_state()
{
	...
	sandbox_state.with_xml_node([&] (Xml_node const &sandbox_state) {
		...
		case State::CONTROLS_SECURITY_USER_PASSPHRASE:
			...
			{
				if (_child_provides_session(sandbox_state, _rump_vfs, "File_system")) {
					client_fs_in_use = true;
				}
			}
			switch (_resizing_state) {
	...
	if (_client_fs_in_use != client_fs_in_use) {

		_client_fs_in_use = client_fs_in_use;
		update_dialog = true;
	}
	if (update_dialog) {
...
void File_vault::Main::produce_xml(Xml_generator &xml)
{
			...
			xml.node("frame", [&] () {
				...
				xml.node("vbox", [&] () {
						...
						[&] (Xml_generator &xml)
					{
						...
						{
							if (_client_fs_in_use) {

								gen_info_line(xml, "Info 1", "Cannot expand client FS while in use!");
				...
				xml.node("vbox", [&] () {
						...
						[&] (Xml_generator &xml)
					{
						...
						case Resizing_state::RUN_RESIZE2FS:
							...
							class Unexpected_resizing_state { };
							throw Unexpected_resizing_state { };
						}
					});
				});
			});
	...
	case State::SHUTDOWN_WAIT_TILL_DEINIT_REQUEST_IS_DONE:
		...
		gen_titled_info_frame(xml, "1", _shutdown_title, "Please wait...", MAIN_FRAME_WIDTH);
		break;
	}
}
