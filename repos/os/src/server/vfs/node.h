                            ...
                            public Vfs::Io_response_handler
{
	private:
		...
		seek_off_t _initial_seek_offset { 0 };
	...
	protected:
		...
		Submit_result _submit_write_at(file_offset seek_offset)
		{
			...
			_initial_seek_offset = seek_offset;
		 ...
		 */
		file_size _execute_write(char const *src_ptr, size_t length, seek_off_t write_pos)
		{
			file_size out_count = 0;
			try {
				_handle.seek(_initial_seek_offset + write_pos);
			...
			case Packet_descriptor::WRITE:
				{
					...
					size_t       const consumed = _execute_write(src_ptr, count, _write_pos);
