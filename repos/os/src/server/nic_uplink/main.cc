...
struct Byte_range
{
	...
	void print(Output &out) const
	{
		...
		enum { MAX_BYTES_PER_LINE = 16 };
		enum { MAX_BYTES_PER_WORD = 8 };
...
class Net::Network_interface
{
	...
	public:
		...
		void send_pkt(Genode::size_t pkt_size, GENERATE_PKT && generate_pkt)
		{
			...
			_source.alloc_packet_attempt(pkt_size).with_result(
				[&] (Packet_descriptor pkt)
				{
					...
					log("--------------------");
					log(Byte_range { (uint8_t *)pkt_base, pkt_size });
					log("--------------------");
		...
		void for_each_received_pkt(HANDLE_PKT && handle_pkt) const
		{
...
{
	_net_if.release_acked_pkts();
	_net_if.for_each_received_pkt([&] (Byte_range_ptr const &src) {
		Size_guard size_guard { src.num_bytes };
		Ethernet_frame &eth { Ethernet_frame::cast_from(src.start, size_guard) };
		log("rcv ", eth);
	});
	_net_if.wakeup_peers();
}
