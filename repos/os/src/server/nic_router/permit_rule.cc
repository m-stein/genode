...
Permit_rule::Permit_rule(Policy_tree &policies, Xml_node const &node)
:
...
{
	Genode::print(output, "requests to ", _policy);
}


Permit_any_rule::Permit_any_rule(Policy_tree &policies, Xml_node const &node)
:
	Permit_rule(policies, node)
{ }
...
{
	Genode::print(output, "port ", _port, " requests to ", _policy);
}


Permit_single_rule::Permit_single_rule(Policy_tree    &policies,
                                       Xml_node const &node)
:
	Permit_rule(policies, node),
