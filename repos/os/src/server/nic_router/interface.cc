...
{
	switch (prot) {
	case Tcp_packet::IP_ID: return _policy.tcp_forward_rules();
	case Udp_packet::IP_ID: return _policy.udp_forward_rules();
...
{
	switch (prot) {
	case Tcp_packet::IP_ID: return _policy.tcp_rules();
	case Udp_packet::IP_ID: return _policy.udp_rules();
	default: throw Bad_transport_protocol(); }
}
...
void Interface::_adapt_eth(Ethernet_frame          &eth,
                           ...
                           Interface               &interface)
{
	Ipv4_address const &hop_ip = interface._policy.next_hop(ip);
                                   ...
                                   Interface           &interface)
{
	...
	try {
		Nat_rule &nat = interface._policy.nat_rules().find_by_policy(_policy);
		if(_config().verbose()) {
                           ...
                           Packet_descriptor const &pkt)
{
	...
	try {
		Link_side &local_side = _links(prot).find_by_id(local);
	...
	if (local.dst_ip == _router_ip()) {
		try {
			...
			Interface &interface = rule.policy().interface().deref();
			if(_config().verbose()) {
	...
	try {
		...
		Interface &interface = permit_rule.policy().interface().deref();
		if(_config().verbose()) {
	...
	try {
		Ip_rule const &rule =
			_policy.ip_rules().longest_prefix_match(local.dst_ip);
...
{
	return _policy.interface_attr().address;
}
	...
	_policy(policy)
{
	if (_config().verbose()) {
		...
		log("  Router identity: MAC ", _router_mac, " IP ",
		    _router_ip(), "/", _policy.interface_attr().prefix);
	}
	_policy.interface() = *this;
}
...
Configuration &Interface::_config() const { return _policy.config(); }


void Interface::print(Output &output) const
{
	Genode::print(output, "\"", _policy.label(), "\"");
}
