                                             ...
                                             Domain         &local_domain)
{
	...
	if (!dhcp_srv.alloc_any_ip(ip))
		return Packet_error::drop("failed to allocate IP for DHCP client");
	...
	catch (Out_of_ram)  { return Packet_error::drop("out of RAM while creating DHCP allocation"); }
	catch (Out_of_caps) { return Packet_error::drop("out of CAPs while creating DHCP allocation"); }
	return Packet_ok();
}
