                                            ...
                                            Domain                 &remote_domain)
{
	Packet_result result { };
	Port_allocator_guard *remote_port_alloc_ptr { };
	Port remote_src_port = _src_port(prot, prot_base);
	Ipv4_address remote_src_ip = ip.src();
                                      ...
                                      Ipv4_address_prefix const &local_intf)
{
	...
	if (icmp.checksum_error(size_guard.unconsumed())) {
		log("bad ICMP checksum"); ASSERT_NEVER_REACHED; }
...
char buf[1600];
ASSERT(pkt.size() <= 1600);
Genode::memcpy(buf, _sink.packet_content(pkt), pkt.size());
	...
	{
ASSERT(Genode::memcmp(buf, _sink.packet_content(pkt), pkt.size()) == 0);
...
{
auto result = _handle_arp(eth, size_guard, local_domain);
if (result.type == Packet_result::INVALID) { log(__func__,__LINE__); } return result;
}
		case Ethernet_frame::Type::IPV4:
{
auto result = _handle_ip(eth, size_guard, pkt, local_domain);
if (result.type == Packet_result::INVALID) { log(__func__,__LINE__); } return result;
}
