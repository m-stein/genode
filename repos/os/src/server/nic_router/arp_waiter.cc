...
:
	_src_le(this), _src(src), _dst_le(this), _dst_ptr(dst_), _ip(ip),
	_packet(packet)
{
	_src.own_arp_waiters().insert(&_src_le);
	dst().foreign_arp_waiters().insert(&_dst_le);
}
...
Arp_waiter::~Arp_waiter()
{
	...
	dst().foreign_arp_waiters().remove(&_dst_le);
}


void Arp_waiter::handle_config(Domain &dst_)
{
	dst().foreign_arp_waiters().remove(&_dst_le);
	_dst_ptr = Pointer<Domain>(dst_);
	dst().foreign_arp_waiters().insert(&_dst_le);
}
...
{
	Genode::print(output, "IP ", _ip, " DST ", dst());
}
