...
Permit_any_rule *Transport_rule::_read_permit_any(Policy_tree    &policies,
                                                  Xml_node const &node,
                                                  Allocator      &alloc)
{
	try {
		Xml_node sub_node = node.sub_node("permit-any");
		return new (alloc) Permit_any_rule(policies, sub_node);
	}
...
Transport_rule::Transport_rule(Policy_tree    &policies,
                               Xml_node const &node,
                               Allocator      &alloc,
...
:
	Direct_rule(node),
	_permit_any(_read_permit_any(policies, node, alloc))
{
	...
	node.for_each_sub_node("permit", [&] (Xml_node const &node) {
		try {
			Permit_single_rule &rule = *new (alloc)
				Permit_single_rule(policies, node);
