...
class Platform_timer
{
	private:
		...
		using duration_t = Kernel::duration_t;
		...
		duration_t const        _max_timeout_us;
	...
	public:
		...
		:
			...
			_last_timeout_us(0), _max_timeout_us(Kernel::timeout_max())
		{
		...
		unsigned long curr_time() const
		{
			...
			duration_t const passed_us = Kernel::timeout_age_us();
		 ...
		 */
		duration_t max_timeout() const { return _max_timeout_us; }

		/**
		 * Return minimum timeout in microseconds
		 */
		static duration_t min_timeout() { return 1000; }
		 ...
		 */
		void schedule_timeout(duration_t timeout_us)
		{
			Genode::Lock::Guard lock(_curr_time_us_lock);
