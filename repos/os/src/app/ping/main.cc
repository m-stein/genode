...
Xicroseconds read_sec_attr(Xml_node const  node,
                           char     const *name,
             ...
             public Dhcp_client_handler
{
	private:
		...
		Xicroseconds                    _send_time     { 0 };
		Xicroseconds                    _period_us     { read_sec_attr(_config, "period_sec", (uint64_t)DEFAULT_PERIOD_SEC) };
                                   ...
                                   Size_guard  &size_guard)
{
	...
	uint64_t time_us = _timer.curr_time().xrunc_to_plain_us().value - _send_time.value;
...
void Main::_send_ping(Duration)
{
	...
	_send_time = _timer.curr_time().xrunc_to_plain_us();
}
