...
class Main
{
	private:
		...
		Timer::Connection      _tic_timer_connection;
		Genode::Timer          _timer;
...
//		One_shot_timeout<Main> _ot2 { _timer, *this, &Main::_handle_ot2 };
	...
	public:
		...
		Main(Env &env) : _timer_connection(env), _tic_timer_connection(env),
		                 _timer(_timer_connection, _tic_timer_connection, env.ep())
		{
