...
.macro _mm_save_normal_state
	...
	mov   lr, #0                    /* switch to secure state                */
...
.macro _mm_switch_to_secure nr, off
	...
	srsdb sp, #0x16             /* store return state on mon_sp              */
...
_mm_switch_to_normal:
	...
	mov   lr, #1                /* switch to non-secure mode                 */
	mcr   p15, 0, lr, c1, c1, 0
	isb                         /* instruction barrier                       */
	ldr   lr, [sp, #-4]
...
/**
 * Start with exception numbers after the ones for the secure world's
 * exception mode, so we are compatible with pre-existing kernel
 * routines.
 */
_mm_smc_entry:        _mm_switch_to_secure 8  0 /* start behind smc inst.    */
_mm_pf_abort_entry:   _mm_switch_to_secure 9  4 /* replay instruction        */
_mm_data_abort_entry: _mm_switch_to_secure 10 4 /* replay instruction        */
_mm_irq_entry:        _mm_switch_to_secure 11 4 /* replay instruction        */
_mm_fiq_entry:        _mm_switch_to_secure 12 4 /* replay instruction        */
