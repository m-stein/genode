...
/* Boot-modules-region constraints */
...
/* Function that provides address regions by ID */
typedef Native_region * (*Region_pool)(unsigned const);

/* Get range of mode transition region for every PD */
namespace Kernel
{
...
{
	/* Initialise platform resource allocators. Core mem alloc must come
	 * first because it is used by the other allocators */
	enum { VERBOSE = 0 };
