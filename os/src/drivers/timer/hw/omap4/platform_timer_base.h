...
#ifndef _PLATFORM_TIMER_BASE_H_
#define _PLATFORM_TIMER_BASE_H_
...
namespace Genode
{
	/* FIXME this is board dependent (omap4 is only the SOC spec) */
	enum { PANDA_A2_SYS_CLK = 38400000};
	...
	class Omap4_gp_timer_1 : public Mmio
	{
		...
		static float tics_per_us() {
			return (float)PANDA_A2_SYS_CLK / 1000000; }
...
//				unsigned i = 0;
//				while(1) {
//					run_and_wrap(us_to_tics(1000000));
//					while(!read<Irqstatus::Ovf_it_flag>());
//					PINF("TIC %u", i++);
...
//				while(1);
			...
			{
				Tcrr::access_t const v = read<Tcrr>();
				wrapped = (bool)read<Irqstatus::Ovf_it_flag>();
				return wrapped ? read<Tcrr>() : v;
			}
	};
}
                            ...
                            public Genode::Omap4_gp_timer_1
{
	public:
		 ...
		 */
		Platform_timer_base() :
			Io_mem_connection(GP_TIMER_3_MMIO_BASE,
			                  GP_TIMER_3_MMIO_SIZE),

			Genode::Omap4_gp_timer_1((Genode::addr_t)Genode::env()->rm_session()->attach(dataspace()))
		{ }
};

#endif /* _PLATFORM_TIMER_BASE_H_ */
